...
#   gh release download v0.1.0 -R duration-ai/bonsai-image-ios -p vae.safetensors -D bonsai-model
```
...
2. **File ▸ Add Package Dependencies… ▸ Add Local…** → this checkout's
   `bonsai-image-ios` folder; add the **`BonsaiEngine`** product to the target.
